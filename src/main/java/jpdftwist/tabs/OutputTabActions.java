...
public class OutputTabActions extends ActionTab {
    ...
    public OutputTabActions(MainWindow mainWindow) {
        ...
        outputTab.setTempFileListener(e -> mainWindow.getInputTab().setUseTempFiles(outputTab.isTempFilesComboBoxSelected()));
    }
    ...
    public void checkRun() {
        ...
        if (mainWindow.getInputTab().getBatchLength() > 1) {
            if (!outputFileName.contains("<F>") && !outputFileName.contains("<FX>") && !outputFileName.contains("<P>")
                ...
                showException(new IOException("Variables in output file name required for batch mode"), "Variables in output file name required for batch mode");
            }
        }
    }
